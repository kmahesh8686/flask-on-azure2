...
# 🚫 Script content to replace /WebResource.axd call
WEBRESOURCE_AXD_CONTENT = """
...
def fetch_from_proxy():
    ...
    if result_container["content"]:
        ...
        # 🚫 Remove /WebResource.axd
        for script_tag in soup.find_all("script", src=True):
            if "WebResource.axd" in script_tag["src"]:
                script_tag.decompose()

        # ✅ Inject static replacement script
        custom_script_tag = BeautifulSoup(WEBRESOURCE_AXD_CONTENT, 'html.parser')
        if head:
            head.append(custom_script_tag)
