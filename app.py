...
def admin_dashboard():
    ...
    token_list_html = "".join(
        f"<li style='margin:8px 0;'><a href='#' onclick=\"loadToken('{t}','otp')\" style='color:#2980B9;text-decoration:none;font-weight:700;'>{t}</a></li>"
        for t in PREDEFINED_TOKENS
    )

    limit_token_list_html = "".join(
        f"<li style='margin:8px 0;'><a href='#' onclick=\"loadLimit('{t}')\" style='color:#2980B9;text-decoration:none;font-weight:700;'>{t}</a></li>"
        for t in PREDEFINED_TOKENS
    )
    ...
    <head>
        ...
        <style>
            ...
            a.linkbutton {{ color:white; background:#E74C3C; padding:10px 12px; display:inline-block; border-radius:6px; text-decoration:none; margin-top:10px; }}
        </style>
        <script>
            function loadToken(token, section) {
                var url = '/status/' + token + '?embed=1&section=' + section;
                document.getElementById('content_panel').innerHTML = '<div class="card">Loading...</div>';
            ...
            window.onload = function() { /* show nothing until user selects */ };
        </script>
    </head>
    <body>
        <div class="container">
            <div class="sidebar">
                <h2>ADMIN</h2>
                <div style="margin-bottom:10px;font-weight:700">TOKENS</div>
                <ul class="token-list">{token_list_html}</ul>
                <div style="margin-top:14px;font-weight:700">LIMIT EXCEEDED</div>
                <ul class="token-list">{limit_token_list_html}</ul>
                <div style="margin-top:14px;font-weight:700">TOKEN CAP</div>
                <a href='#' onclick="document.getElementById('content_panel').innerHTML='';fetch('/admin?panel=cap').then(function(r){return r.text();}).then(function(t){document.getElementById('content_panel').innerHTML=t;})" style='color:#fff;text-decoration:none;margin-top:8px;'>Open Caps Panel</a>
                <div style="margin-top:auto;"> 
                    <a href="/admin-logout" class="linkbutton">LOGOUT</a>
                </div>
            </div>
            <div class="content">
                <div id="content_panel">
                    <div class="card"><h3>Welcome, Admin</h3><p>Select a token or a section from the left to load fresh data here.</p></div>
                </div>
            </div>
        </div>
    </body>
...
# =========================
# Admin - limit-exceeded per token (with embed)
# =========================
    ...
    <head>
        ...
        <script>
            function loadSection(section) {
                fetch(window.location.pathname + '?embed=1&section=' + section, {credentials:'same-origin'})
                    .then(function(r){ return r.text(); }).then(function(html){document.getElementById('right_panel').innerHTML = html;});
            }
